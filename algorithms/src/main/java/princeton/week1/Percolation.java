...
public final class Percolation {
    ...
    public boolean isFull(int row, int col) {
        int p = (row - 1) * n + (col - 1);
        int q = virtualTopSite;

        return isOpen(row, col) &&
                ufDataStructureWithOneExtraSite.find(p) == ufDataStructureWithOneExtraSite.find(q);
    }
    ...
    public boolean percolates() {
        int p = virtualTopSite;
        int q = virtualBottomSite;

        return n != 1 ? ufDataStructureWithTwoExtraSites.find(p) == ufDataStructureWithTwoExtraSites.find(q) : grid[0][0];
    }
